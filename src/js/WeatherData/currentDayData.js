...
const pixabayKey = '39735160-014c6c17620a2b57de6626257';
...
// const currentTime = document.querySelector('time__hour');
...
async function getWeatherForToday() {
  try {
    const response = await axios.get(baseUrlForTodayWeather + weatherData.city);
...
async function getCityBackground(cityName) {
  ...
  try {
    const response = await axios.get(
      pixabayURL + requestParameters + '&key=' + pixabayKey
    );

    if (response.data.hits && response.data.hits.length > 0) {
      const randomImg = Math.floor(Math.random() * response.data.hits.length);
      const img = response.data.hits[randomImg].largeImageURL;
      bg.style.backgroundImage = `url(${img})`;
    } else {
      const buildingsRequestParameters = `?image_type=photo&category=buildings&orientation=horizontal&q=&page=1&per_page=40`;
      const buildingsResponse = await axios.get(
        pixabayURL + buildingsRequestParameters + '&key=' + pixabayKey
      );

      if (
        buildingsResponse.data.hits &&
        buildingsResponse.data.hits.length > 0
      ) {
        const randomImg = Math.floor(
          Math.random() * buildingsResponse.data.hits.length
        );
        const img = buildingsResponse.data.hits[randomImg].largeImageURL;
        bg.style.backgroundImage = `url(${img})`;
      }
    }
  } catch (error) {
